...
@HiltViewModel
class NotificationOnOffModelView @Inject constructor(
    application: Application, private val repository: CommonRepository
) : AndroidViewModel(application) {
    val progressIndicator = MutableLiveData<Boolean>()
    val errorResponse = MutableLiveData<Throwable>()
    val mNotificationResponse = MutableLiveData<Event<NotificationOnOffResponse>>()
    var context: Context? = null


    fun allNotificationType(acceptBody: NotificationOnOffBody, activity: Activity) {
        //progressDialog.start(activity.getString(R.string.please_wait))
        progressIndicator.value = true
        repository.notificationOnOff(acceptBody).subscribeOn(Schedulers.io()).observeOn(
